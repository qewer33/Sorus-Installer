...
public class MinecraftPathPanel extends BasePanel {
  ...
  public MinecraftPathPanel() {
    ...
    mcdirLabel.setLocation(
        190
            - mcdirLabel.getFontMetrics(mcdirLabel.getFont()).stringWidth("Minecraft Directory")
                / 2,
        215);
  ...
  private void checkErrors(ActionEvent e) {
    try {
      if (!checkMcDirError()) {
        this.displayPanel(new SelectInstallationModePanel(this.getMinecraftPath()));
      } else {
  ...
  // This boolean checks if the path entered is a valid Minecraft directory by checking if it has
